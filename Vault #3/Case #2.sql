...
SELECT prio_team,
       ...
       SUM(issues) AS original_issues,
       ((SUM(issues)) * 0.3) AS issues_what_the_main_team_fixed,
       (((SUM(issues) * 0.7) * 0.1)) AS issues_what_the_backup_team_fixed,
       ((SUM(issues) - ((SUM(issues)) * 0.3) - (((SUM(issues) * 0.7) * 0.1)))) AS difference_in_the_issues,
       SUM(original_dmg) AS original_dmg,
       ((SUM(original_dmg)) * 0.3) AS total_dmg_what_the_main_team_fixed,
       (((SUM(original_dmg) * 0.7) * 0.1)) AS total_dmg_what_the_backup_team_fixed,
       ((SUM(original_dmg) - ((SUM(original_dmg)) * 0.3) - (((SUM(original_dmg) * 0.7) * 0.1)))) AS difference_in_the_dmg
FROM (SELECT prio_team,
             backup_team,
             solar_losses.error_code,
             SUM(loss) AS original_dmg,
...
SELECT prio_team,
       ...
       ((SUM(issues)) * 0.3) AS issues_what_the_main_team_fixed,
       (((SUM(issues) * 0.7) * 0.1)) AS issues_what_the_backup_team_fixed,
       (SUM(issues) - ((SUM(issues)) * 0.37)) AS difference_in_the_issues,
       SUM(original_dmg) AS original_dmg,
       ((SUM(original_dmg)) * 0.3) AS total_dmg_what_the_main_team_fixed,
       (((SUM(original_dmg) * 0.7) * 0.1)) AS total_dmg_what_the_backup_team_fixed,
       (SUM(original_dmg) -((SUM(original_dmg)) * 0.37)) AS difference_in_the_dmg
FROM (SELECT prio_team,
             backup_team,
             solar_losses.error_code,
             SUM(loss) AS original_dmg,
